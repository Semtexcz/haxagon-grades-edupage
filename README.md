...
pipx install "git+https://github.com/Semtexcz/EduPageAutomat.git"

# stažení Playwright prohlížeče (spustí se v pipx prostředí balíčku)
pipx run --spec "EduPageAutomat" playwright install firefox

# ověření instalace
edupage --help
```

Aktualizace na poslední verzi z repozitáře: `pipx install --force "git+https://github.com/Semtexcz/EduPageAutomat.git"`.
...
```bash
poetry run python -m edu_page_automat.cli --help
```
...
poetry run python -m edu_page_automat.cli list

# přinutit nové přihlášení + uložit session
poetry run python -m edu_page_automat.cli login

# spustit scénář na vytvoření písemky
poetry run python -m edu_page_automat.cli create-task --class "3.gpu" --task "Test 1:50" --task "Opakování:30"

# otevřít stránku se známkami pro daný školní rok
poetry run python -m edu_page_automat.cli grades --year 2025
```
...
Každý scénář je implementován ve složce `src/edu_page_automat/scenarios/` a registruje vlastní CLI příkaz přes metodu `register_cli`.
...
- Sdílená logika pro práci se session je v `src/edu_page_automat/auth_manager.py`.
