...
The exported CSV keeps task categories from EduPage headers such as `Dan - Frontend`. When EduPage shows grades in a display form like `m · 20` or `15 · 20`, the export writes only the fill-compatible leading value. `--task-category` can restrict the export to one exact EduPage task category so later diff generation can focus on that subset. The remaining required headers are compatible with `fill-grades`, so the file can be reviewed, edited, and used later as grade input.
...
Rows with empty EduPage-style source-of-truth points are counted but not emitted because they cannot be represented as a fill operation. Empty raw Google Classroom point values are emitted as `m`. EduPage-style CSV inputs also accept the export display form `value · max` and normalize it to the leading fill-compatible value. With `--keep-better-current`, the command treats `m` as `0`, keeps the current EduPage value when it is higher than the source-of-truth value, and writes a `*-kept-current.csv` report listing those preserved rows. Rows missing from the current EduPage export are counted as review items instead of being emitted.
